...
#import "JINS_MEME_DataLogger-Swift.h"
...
- (void)viewDidLoad {
    ...
    // This is where the socket connection is established to the server
    
    
    // connect to the right address
    url = [[NSURL alloc] initWithString:@"http://localhost:3000"];
    // create socket connection
    socketIO = [[SocketIOClient alloc] initWithSocketURL:url config:@{@"log": @NO}];
    // connect socket
...
- (void)DataAnalysis {
    ...
    if (DataBuff[0] == 0x14) {
        NSArray *socketValues; //variable for sending values via socket
        NSMutableString* socketValueHolder; // added to SDK implementation
        switch (DataBuff[1]) {
            case 0x81:
                revision = DataBuff[4];
                minor = DataBuff[5];
            ...
            case 0x98:
                ...
                eogH1 = eogL1 - eogR1; // We calculate the horizonal EOG value by subtracting the value returned from the left nosepad electrode (L) from the right (R).
                eogV1 = (0 - ((eogL1 + eogR1) / 2)); // The vertical signal is calculated by subtracting the value from the bridge electrode (B) from the average of the two nosepad electrode. But there is no bridge value...
                eogH2 = eogL2 - eogR2; //We calculate the horizonal EOG value by subtracting the value returned from the left nosepad electrode (L) from the right (R).
                eogV2 = (0 - ((eogL2 + eogR2) / 2)); // The vertical signal is calculated by subtracting the value from the bridge electrode (B) from the average of the two nosepad electrode. But there is no bridge value...
                /*
                 * left this part for debugging
                 */
                //NSLog(@"Pointer = %hd", eogH1);
                /*if(eogV1 >= 200){
                    printf("looked up or down");
                }
                if(eogH1 >= 100){
                    printf("looked left or right");
                }*/
                
                
                socketValueHolder = [NSMutableString stringWithFormat:@"H1:%d", eogH1];    // does not need to be released. Needs to be retained if you need to keep use it after the current function.
                [socketValueHolder appendFormat:@",V1:%d", eogV1];
                [socketValueHolder appendFormat:@",H2:%d", eogH2];
                [socketValueHolder appendFormat:@",V2:%d", eogV2];
                
                // let's add timestamp
                CFAbsoluteTime timeInSeconds = CFAbsoluteTimeGetCurrent();
                [socketValueHolder appendFormat:@",t:%f", timeInSeconds];

                
                socketValues = @[socketValueHolder];
                
                [socketIO emit:@"message" withItems:socketValues];
